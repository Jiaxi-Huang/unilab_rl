...
class SonicBackbone(nn.Module):
    ...
    def _encode_masked(
        ...
        """Encode only active rows, returning a batch-aligned token tensor."""
        batch_size = reference.shape[0]
        ...
        encoded = self._encode(name, active)
        # Autocast may run the encoder in bf16/fp16 while replay observations
        # remain fp32.  Indexed assignment requires exact dtype parity, so the
        # batch-aligned buffer must follow the encoded tensor rather than the
        # source observation.
        result = encoded.new_zeros(
            (batch_size, self.config.num_tokens, self.config.token_dim)
        )
        # CopySlices preserves the gradient from the active encoder rows.
        result[mask] = encoded
        return result
    ...
    def _validate_inputs(
        ...
        if encoder_index.shape != (batch_size, 2):
            raise ValueError(f"encoder_index must have shape (B, 2), got {encoder_index.shape}")
